...
void MainWindow::datenbankConnect()
{
    ...
    // Tabelle erstellen
    query = QSqlQuery(db);
    if(!query.exec("CREATE TABLE vorfahren (id INTEGER PRIMARY KEY, vorname TEXT, nachname TEXT, geburtsname TEXT, geburtsdatum DATE, geburtsort TEXT, gestorben DATE, begraben TEXT, beruf TEXT, kinder INTEGER, vater TEXT, mutter TEXT)")){
        qWarning() << "ERROR query beim erstellen: " << query.lastError().text();
    }
}

void MainWindow::datenbankInput(QString vn, QString nn, QString gn, QString gd, QString go, QString ge, QString be, QString bf, int k, QString v, QString m)
{
    // Daten in die Tabelle eingeben (aktuell noch feste Werte)
    query.prepare("INSERT INTO vorfahren (vorname, nachname, geburtsname, geburtsdatum, geburtsort, gestorben, begraben, beruf, kinder, vater, mutter) VALUES(:vorname, :nachname, :geburtsname, :geburtsdatum, :geburtsort, :gestorben, :begraben, :beruf, :kinder, :vater, :mutter)");
...
void MainWindow::datenbankUpdate()
{
    ...
    QString vname, nname, gname, vn, nn, gn, gebOrt, begraben, beruf;
    int anzKinder;
    ...
    query.exec("UPDATE vorfahren SET vorname = '" + vn + "', nachname = '" + nn + "', geburtsname = '" + gn + "', geburtsort = '" + gebOrt + "', begraben = '" + begraben + "', beruf = '" + beruf + "', kinder = 7, vater = '" + vater + "', mutter = '" + mutter + "' WHERE vorname = '" + vname + "' AND nachname = '" + nname + "'");
...
void MainWindow::on_button_accept_clicked()
{
    ...
    if (ui->radioButton_neu->isChecked()) {
        ...
        QString vorname, nachname, geburtsname, text, geburtsdatum, gestorben;
        ...
        if(eingabenKorrekt){
            datenbankInput(vorname, nachname, geburtsname, geburtsdatum, geburtsort, gestorben, begraben, beruf, anzKinder.toInt(), vater, mutter);
